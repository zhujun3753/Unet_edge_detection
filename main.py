...
def main(args):
    ...
    if not args.is_testing:
        if args.resume and checkpoint_path is not None:
            model.load_state_dict(torch.load(checkpoint_path, map_location=device))
            if os.path.exists(checkpoint_path.replace('model.pth', 'ecn_model.pth')):
                ecn_model.load_state_dict(torch.load(checkpoint_path.replace('model.pth', 'ecn_model.pth'), map_location=device))
    ...
    if args.is_testing:
        ...
        if args.double_img:
            ...
            test(checkpoint_path, dataloader_val, model,ecn_model,  device, output_dir, args)
    ...
    for epoch in range(ini_epoch, args.epochs):
        ...
        torch.save(ecn_model.module.state_dict() if hasattr(ecn_model, "module") else ecn_model.state_dict(), os.path.join(output_dir_epoch, '{0}_ecn_model.pth'.format(epoch)))
